...
def parse_args():
    ...
    parser.add_argument("--task_dir", default="./preprocess", type=str)
...
def build_gym(args):
    ...
    for filename in sorted(os.listdir(args.task_dir)):
        if filename.endswith(".py") and not filename.startswith("0") and not filename.startswith("_") and \
                filename!="preprocess_util.py" and filename!="modify_dataset.py" and "unifiedqa" not in filename:
            all_tasks.append(filename)
    print(all_tasks)
    print(ALL_TASKS)
    assert all_tasks == ALL_TASKS
    print("Passing file checks ...")
    all_tasks = [os.path.join("preprocess", task) for task in all_tasks]
